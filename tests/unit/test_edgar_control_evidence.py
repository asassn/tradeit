...
def test_the_shipped_state_measures_15_unresolved_and_17_awaiting_verification() -> None:
    ...
    exactly that for MSFT, CSCO, AMZN, SPY, QQQ, ETYS, WBVN and KOOP in turn,
    ...
    assert len(unresolved_controls(evidence)) == 15
    assert len(controls_awaiting_manual_verification(evidence)) == 17
    assert sum(1 for r in resolved if r.counts_in_numerator) == 15
    assert sum(1 for r in resolved if r.is_manually_verified) == 13
