...
def test_the_shipped_state_measures_10_unresolved_and_12_awaiting_verification() -> None:
    ...
    EXDS, PSIX and GCTY in turn, which is the behaviour rather than a nuisance.
    """
    evidence = load_control_evidence(DEFAULT_EVIDENCE_PATH)
    resolved = resolve_controls(evidence)

    assert len(resolved) == 30
    assert len(unresolved_controls(evidence)) == 10
    assert len(controls_awaiting_manual_verification(evidence)) == 12
    assert sum(1 for r in resolved if r.counts_in_numerator) == 20
    assert sum(1 for r in resolved if r.is_manually_verified) == 18
