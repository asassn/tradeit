...
| **0b** — 30 controls to `MANUAL_VERIFIED` | **in progress; 23 of 30 still require `MANUAL_VERIFIED`.** Two measurements, and they are not the same number. **Identity:** 9/30 have evidence-backed resolutions — 7 `MANUAL_VERIFIED` (IPET, TGLO, ENE, BEL, LEH, MSFT, CSCO) and 2 `RESOLVED` (AAPL, GM) — leaving **21/30 `UNRESOLVED`**. **Milestone completion:** only `MANUAL_VERIFIED` clears this milestone, so the 2 `RESOLVED` controls still count as outstanding and **7/30 are done, 23/30 remain**. **0b completes only at 30/30 `MANUAL_VERIFIED`.** A `RESOLVED` control has a defensible mapping from a dated primary source; `MANUAL_VERIFIED` additionally means a person read a filing and cited it, which is what this milestone asks for. **No CIK has been guessed** — every mapping cites a primary source, and the fixture in `controls.py` still carries none. Both counts come from `tradeit edgar controls`, which prints them separately; do not restate either from memory |
