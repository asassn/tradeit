...
| **0b** — 30 controls to `MANUAL_VERIFIED` | **in progress; 10 of 30 still require full adjudication.** Three measurements, and they are not the same number. **Identity:** 23/30 have evidence-backed resolutions — 21 `MANUAL_VERIFIED` and 2 `RESOLVED` (AAPL, GM) — leaving **7/30 `UNRESOLVED`**. **Mapping quality:** 21/30 have every *recorded* mapping at `MANUAL_VERIFIED` (IPET, TGLO, ENE, BEL, LEH, MSFT, CSCO, AMZN, SPY, QQQ, ETYS, WBVN, KOOP, MPPP, WCOM, EXDS, PSIX, GCTY, BCST, CPQ, BBBY). **Milestone completion:** **20/30 are fully adjudicated, 10/30 remain**, and 0b **completes only at 30/30 fully adjudicated**. A control is fully adjudicated when every recorded mapping is `MANUAL_VERIFIED` **and** its issuer question is settled — three controls (BBBY, GM, AOL) must adjudicate whether a *second* issuer held their ticker, discharged either by evidencing one or by a cited finding that none was established. BBBY is currently mapping-verified but not adjudicated: one impeccably cited mapping, second-issuer question still open. That gap is the `full-01` failure mode, so it must not hide inside a completion count. **No CIK has been guessed** — every mapping cites a primary source, and the fixture in `controls.py` still carries none. All three counts come from `tradeit edgar controls`, which prints them separately; do not restate any from memory |
