...
every correction, is in [`SIGNAL_SCOREBOARD.md`](SIGNAL_SCOREBOARD.md) §1–§48 and
...
Ledger: **138 trials**; hurdle |t| > 2.5702 on the normal scale, restated with `small_sample_hurdle` for tests built from few blocks (3.0967 at twelve). Every result above is
