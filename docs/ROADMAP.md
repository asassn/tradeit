...
| 6 | Fundamentals & earnings quality | In progress. **The denominator has been built and run** — 90,548 registrant timelines, 29,180 confirmed dated exits — and the survivorship gate returned **`SURVIVOR_BIASED`** on every run until 2026-09-12, when the owner adopted §5's denominator and it read **`PARTIALLY_SURVIVORSHIP_CORRECTED`** — 8,371 of 21,618 dated Exchange Act exits priced, 38.7%. On 2026-09-15, after one month of **Sharadar** prices was bought and landed for 1,666 identity-resolved exits, it reads **`MATERIALLY_SURVIVORSHIP_CORRECTED`** — 10,038 priced, 46.4%; a day later, after 1,206 more landed under vendor-derived identity, **11,249 priced, 52.0%**. Both readings are published side by side. **On 2026-09-15 the owner lifted the standing rule with four stated limitations** (`EDGAR_DELISTING_DENOMINATOR.md` §7e): thin pre-2007 coverage, 4.23% of raw prints withheld around contradicted splits, unrecorded splits not caught, and every earlier result to be re-run before it is relied on. **EODHD is subscribed** (personal use, $19.99/month) and supplies most price bars and corporate actions; Sharadar supplies those for the exits EODHD could not price; everything else is free SEC EDGAR. Its fundamentals endpoint returns 403 on this plan, so **the fundamentals spine was built from the SEC Financial Statement Data Sets instead** — imported, indexed for cross-sectional reads, and effectively beginning 2011Q3 rather than 2009 because the earlier archives are headers. Identity is being seeded for the registrants both original cohorts could not see, and their tickers resolved from filing text alone. Splice adjudication places disputed bars outside their security's interval rather than deleting them. **Counts move daily and are deliberately not restated here** — measure them, or read [`RESEARCH_01_DATA_CONTRACT.md`](RESEARCH_01_DATA_CONTRACT.md) |
| — | **Corpus query views** | **✅ Built 2026-09-08.** `v_prices`, `v_prices_raw`, `v_security_tickers`, `v_dead_registrants`: SQL views that encode the read conventions so `select * from v_prices` is correct by construction — one row per session, adjusted, with disputed and non-session bars already excluded. Measured first: the corpus is **not** slow (a full 9,237-bar series reads in 0.02s, 0% free space, indexes correct), so this is an *interface* change, not a performance one, and it adds no rows and alters none. [`RESEARCH_01_DATA_DICTIONARY.md`](RESEARCH_01_DATA_DICTIONARY.md) stays as the explanation of why. Doubles as the dashboard's data layer. **Do this last, before the next milestone starts** |
| — | **Multi-Timeframe & Portfolio Mandate Architecture** | **Specified and now partly enforced.** [`MULTI_TIMEFRAME_MANDATES.md`](MULTI_TIMEFRAME_MANDATES.md) is written; `src/tradeit/portfolio/mandate.py` turns its §4 table into data, refuses timeframes a mandate does not admit, and makes the §4 separation rule executable. **Still owed:** the `intraday-01` corpus, the `bar_status` contract of §5 (deferred deliberately — the system ingests only completed bars, so the column would be constant, and the migration must wait for the write pipeline to be idle), and the regular/extended-hours declaration (§3.3). May overlap Phase 6; **must complete before Phase 7** |
| — | **Strategy Definition / Builder Architecture** | **Specified and now partly enforced.** [`STRATEGY_BUILDER.md`](STRATEGY_BUILDER.md) is written; `StrategyConfig` was already the versioned, digest-identified definition of §2–§3, and `src/tradeit/strategy/lifecycle.py` adds §5's nine states — promotion one rung with a citation, demotion any distance, an edit dropping to `DRAFT` with no inherited evidence, and `LIVE` refused without the ADR-0004 interlock. `src/tradeit/strategy/validate.py` performs `DRAFT → VALIDATED` against the pattern vocabulary, the detector registry and the mandate, and `src/tradeit/strategy/admissibility.py` applies §5's corpus condition — a version may enter `BACKTESTING` on a survivor-biased corpus and may not climb out of it. **Still owed:** composition from the capability registries (§2) and persistence of versions. Follows the multi-timeframe gate; **must complete before Phase 7** |
| 7 | Opportunity scoring | **Built out of order.** The three scores and the portfolio veto exist and are tested (`src/tradeit/opportunity/`), committed before either gate above. **The first half of that debt is paid**: `src/tradeit/opportunity/slate.py` scopes the ranking to one mandate — a slate carries the mandate and timeframe it was drawn from and refuses to be built from a mixture, through the same `UnitOfStudy` the separation rule uses. **Still owed**: it cannot rank strategies that have no definition, which needs the builder's composition and persistence. Recorded here rather than corrected away — the ordering was authorised and was crossed, and pretending otherwise would hide which part rests on nothing |
...
**71**, measured from `tables.py`. See [`PHASE_02.md`](PHASE_02.md) and
