...
| 6 | Fundamentals & earnings quality | In progress. **Milestone 0b complete — 30/30 controls fully adjudicated.** **No price vendor is selected and none will be bought before Phase 9**: Kibot is eliminated on price (the archive is $990–$2,400, not the ~$14/month assumed), and permanent retention remains a hard requirement that keeps every deletion-on-termination subscription disqualified. Prices are the only dataset here that costs money — filing dates, delisting events, 2009+ fundamentals and macro are free. **The live thread is 0a: the delisting denominator is implemented, wired to the curated control identity, and has never been run against real EDGAR data.** **0d is partly answered and the first answer is adverse: Twelve Data requires deletion of all data within 30 days of termination, derived datasets and research corpora included, so it is eliminated for any permanent corpus.** That settles the question the plan had left open — **`full-01` cannot serve as permanent history**; it stays fully usable and frozen, and it now carries a deletion obligation that fires if the subscription ends. Sharadar and EODHD were already out on licence and Kibot on price, so **FMP and Tiingo, both sent and unanswered, now decide whether any examined vendor satisfies the permanent-retention rule at all** |
