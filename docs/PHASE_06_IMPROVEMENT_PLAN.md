...
| **0b** — 30 controls to `MANUAL_VERIFIED` | **in progress; 9 of 30 still require full adjudication.** Three measurements, and they are not the same number. **Identity:** 23/30 have evidence-backed resolutions — 21 `MANUAL_VERIFIED` and 2 `RESOLVED` (AAPL, GM) — leaving **7/30 `UNRESOLVED`**. **Mapping quality:** 21/30 have every *recorded* mapping at `MANUAL_VERIFIED` (IPET, TGLO, ENE, BEL, LEH, MSFT, CSCO, AMZN, SPY, QQQ, ETYS, WBVN, KOOP, MPPP, WCOM, EXDS, PSIX, GCTY, BCST, CPQ, BBBY). **Milestone completion:** **21/30 are fully adjudicated, 9/30 remain**, and 0b **completes only at 30/30 fully adjudicated**. A control is fully adjudicated when every recorded mapping is `MANUAL_VERIFIED` **and** its issuer question is settled — three controls (BBBY, GM, AOL) must adjudicate whether a *second* issuer held their ticker, discharged either by evidencing one or by a cited finding that none was established. **BBBY is now the first control to discharge that obligation by evidence**: two independently verified issuers (CIK 886158 on Nasdaq, `$.01` par; CIK 1130713 on the NYSE, `$0.0001` par) with `identity_break: true`. Their conformed names differ only by a comma, which is exactly the splice `full-01` fell for — the CIKs, securities and venues are what separate them. GM and AOL remain open. **No CIK has been guessed** — every mapping cites a primary source, and the fixture in `controls.py` still carries none. All three counts come from `tradeit edgar controls`, which prints them separately; do not restate any from memory |
