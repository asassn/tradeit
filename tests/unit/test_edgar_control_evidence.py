...
def test_the_shipped_state_measures_5_unresolved_and_5_awaiting_verification() -> None:
    ...
    EXDS, PSIX, GCTY, BCST, CPQ, BBBY, AOL, GM, AAPL and JDSU in turn, which is
    the behaviour rather than a nuisance.
    """
    evidence = load_control_evidence(DEFAULT_EVIDENCE_PATH)
    resolved = resolve_controls(evidence)

    assert len(resolved) == 30
    assert len(unresolved_controls(evidence)) == 5
    assert len(controls_awaiting_manual_verification(evidence)) == 5
    assert len(controls_awaiting_adjudication(evidence)) == 5
    assert sum(1 for r in resolved if r.counts_in_numerator) == 25
    assert sum(1 for r in resolved if r.is_manually_verified) == 25
    assert sum(1 for r in resolved if r.is_fully_adjudicated) == 25
