...
def test_the_shipped_state_measures_21_unresolved_and_23_awaiting_verification() -> None:
    ...
    exactly that twice, for MSFT and for CSCO, which is the behaviour rather than
    a nuisance.
    """
    evidence = load_control_evidence(DEFAULT_EVIDENCE_PATH)
    resolved = resolve_controls(evidence)
    ...
    assert len(unresolved_controls(evidence)) == 21
    assert len(controls_awaiting_manual_verification(evidence)) == 23
    assert sum(1 for r in resolved if r.counts_in_numerator) == 9
    assert sum(1 for r in resolved if r.is_manually_verified) == 7
