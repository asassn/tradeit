...
2. **The denominator has been run, and its dating rule is broken on 30.7% of
   dated exits.** First real run 2026-08-29: 90,548 registrants, parser-integrity
   gate PASSED on 27,084,668 rows — but `resolve_exit` takes the *earliest*
   confirming filing, so Intel is recorded as exiting in 1994 while still filing
   in 2026. **The per-year curve cannot evaluate a vendor until this is fixed.**
   See milestone 0a and `EDGAR_DELISTING_DENOMINATOR.md` §7bb.
...
| milestone | state |
|---|---|
| **0a** — denominator | **RUN AGAINST REAL EDGAR DATA, 2026-08-29 — and it found a defect.** 129 index files, 1994 Q3–2026 Q3; the parser-integrity gate **PASSED** on all **27,084,668** rows with zero mismatches across form type, CIK, date, path and accession. Measured: **90,548 registrants**, **40,920** confirmed exits carrying a date, **49,628** undated. All 33 handed-over control mappings attached to a registrant. **The defect: `resolve_exit` takes the *earliest* confirming filing, so 12,549 dated exits — 30.7% — are dated BEFORE the registrant's own last periodic report.** `INTEL CORP` is recorded as exiting 1994-08-02 while still filing in 2026; so are Adobe, ConAgra and TJX. This is the inverse of survivorship bias — the corpus invents dead companies rather than omitting them — and a vendor measured against it would be marked down for not supplying delisted history for Intel. **The per-year curve may not be used to evaluate a vendor until this is fixed**; registrant totals, row counts and the undated population are unaffected. Full record, including two falsified predictions, in [`EDGAR_DELISTING_DENOMINATOR.md`](EDGAR_DELISTING_DENOMINATOR.md) §7bb |
...
| **0c** — vendor capability questions | **NOT SENT.** Kibot is eliminated on price, so the questions now belong to whichever candidate is evaluated next; Sharadar and EODHD are untested. The 22 questions are vendor-neutral and sendable verbatim ([`VENDOR_QUESTIONS_READY_TO_SEND.md`](VENDOR_QUESTIONS_READY_TO_SEND.md)). **Question 1 decides any price vendor** — whether "delisted" securities are actually included, or only currently active ones — and it has never been answered by anybody |
...
2. **Sharadar and EODHD are untested candidates**, not eliminations. Neither has
   ...
   `KIBOT_DATA_PROBE.md`.
3. **Milestones: 0b is complete, 0a has never been run against real EDGAR
   data, and 0c is moot now that its vendor is eliminated.** 0a is the live
   thread: it is implemented, wired to the curated control identity, costs
   nothing, and produces the per-year termination denominator — which is also
   the instrument that would *evaluate* any replacement vendor.
