...
| **0b** — 30 controls to `MANUAL_VERIFIED` | **in progress; 25 of 30 still require `MANUAL_VERIFIED`.** Two measurements, and they are not the same number. **Identity:** 7/30 have evidence-backed resolutions — 5 `MANUAL_VERIFIED` (IPET, TGLO, ENE, BEL, LEH) and 2 `RESOLVED` (AAPL, GM) — leaving **23/30 `UNRESOLVED`**. **Milestone completion:** only `MANUAL_VERIFIED` clears this milestone, so the 2 `RESOLVED` controls still count as outstanding and **5/30 are done, 25/30 remain**. **0b completes only at 30/30 `MANUAL_VERIFIED`.** A `RESOLVED` control has a defensible mapping from a dated primary source; `MANUAL_VERIFIED` additionally means a person read a filing and cited it, which is what this milestone asks for. **No CIK has been guessed** — every mapping cites a primary source, and the fixture in `controls.py` still carries none. Both counts come from `tradeit edgar controls`, which prints them separately; do not restate either from memory |
