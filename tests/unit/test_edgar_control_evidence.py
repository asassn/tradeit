...
def test_the_shipped_state_measures_18_unresolved_and_20_awaiting_verification() -> None:
    ...
    exactly that for MSFT, CSCO, AMZN, SPY and QQQ in turn, which is the behaviour
    ...
    assert len(unresolved_controls(evidence)) == 18
    assert len(controls_awaiting_manual_verification(evidence)) == 20
    assert sum(1 for r in resolved if r.counts_in_numerator) == 12
    assert sum(1 for r in resolved if r.is_manually_verified) == 10
