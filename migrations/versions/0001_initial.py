...
def upgrade() -> None:
    ...
    op.create_index('ix_ingestion_recent', 'ingestion_runs', ['dataset', 'started_at'], unique=False)
    op.create_table('instruments',
    sa.Column('instrument_id', sa.BigInteger().with_variant(sa.Integer(), 'sqlite'), autoincrement=True, nullable=False),
    ...
    sa.CheckConstraint("(listing_status = 'active' AND delisted_date IS NULL) OR (listing_status <> 'active' AND delisted_date IS NOT NULL)", name='ck_instrument_lifecycle'),
    ...
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('instrument_id', 'action_type', 'ex_date', 'knowledge_time', name='uq_action_revision')
    )
    op.create_index('ix_action_pit', 'corporate_actions', ['instrument_id', 'ex_date', 'knowledge_time'], unique=False)
    ...
    sa.ForeignKeyConstraint(['instrument_id'], ['instruments.instrument_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('instrument_id', 'fiscal_year', 'fiscal_period', 'knowledge_time', name='uq_earnings_revision')
    )
    op.create_index('ix_earnings_pit', 'earnings_events', ['instrument_id', 'scheduled_date', 'knowledge_time'], unique=False)
    ...
    sa.Column('restatement_of', sa.BigInteger().with_variant(sa.Integer(), 'sqlite'), nullable=True),
    sa.Column('ingested_at', UTCDateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('source', sa.String(length=64), nullable=False),
    ...
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('instrument_id', 'metric', 'fiscal_year', 'fiscal_period', 'knowledge_time', name='uq_fundamental_revision')
    )
    op.create_index('ix_fundamental_pit', 'fundamental_facts', ['instrument_id', 'metric', 'knowledge_time', 'period_end'], unique=False)
    ...
    sa.ForeignKeyConstraint(['instrument_id'], ['instruments.instrument_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('instrument_id', 'timeframe', 'session_date', 'knowledge_time', name='uq_bar_revision')
    )
    op.create_index('ix_bar_pit', 'ohlcv_bars', ['instrument_id', 'timeframe', 'session_date', 'knowledge_time'], unique=False)
    ...
    op.create_index('ix_symbol_instrument', 'symbol_mappings', ['instrument_id', 'valid_from'], unique=False)
    op.create_index('ix_symbol_lookup', 'symbol_mappings', ['ticker', 'valid_from', 'valid_to'], unique=False)
    ...
    sa.Column('ingested_at', UTCDateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('source', sa.String(length=64), nullable=False),
    sa.CheckConstraint('valid_to IS NULL OR valid_to > valid_from', name='ck_universe_interval'),
    sa.ForeignKeyConstraint(['instrument_id'], ['instruments.instrument_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('ix_universe_asof', 'universe_memberships', ['universe', 'valid_from', 'valid_to'], unique=False)
    op.create_index('ix_universe_instrument', 'universe_memberships', ['instrument_id', 'universe'], unique=False)
