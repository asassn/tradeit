...
def test_the_shipped_state_measures_11_unresolved_and_13_awaiting_verification() -> None:
    ...
    EXDS and PSIX in turn, which is the behaviour rather than a nuisance.
    """
    evidence = load_control_evidence(DEFAULT_EVIDENCE_PATH)
    resolved = resolve_controls(evidence)

    assert len(resolved) == 30
    assert len(unresolved_controls(evidence)) == 11
    assert len(controls_awaiting_manual_verification(evidence)) == 13
    assert sum(1 for r in resolved if r.counts_in_numerator) == 19
    assert sum(1 for r in resolved if r.is_manually_verified) == 17
