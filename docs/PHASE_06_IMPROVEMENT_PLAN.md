...
| **0b** — 30 controls to `MANUAL_VERIFIED` | **in progress; 8 of 30 still require full adjudication.** Three measurements, and they are not the same number. **Identity:** 24/30 have evidence-backed resolutions — 22 `MANUAL_VERIFIED` and 2 `RESOLVED` (AAPL, GM) — leaving **6/30 `UNRESOLVED`**. **Mapping quality:** 22/30 have every *recorded* mapping at `MANUAL_VERIFIED` (IPET, TGLO, ENE, BEL, LEH, MSFT, CSCO, AMZN, SPY, QQQ, ETYS, WBVN, KOOP, MPPP, WCOM, EXDS, PSIX, GCTY, BCST, CPQ, BBBY, AOL). **Milestone completion:** **22/30 are fully adjudicated, 8/30 remain**, and 0b **completes only at 30/30 fully adjudicated**. A control is fully adjudicated when every recorded mapping is `MANUAL_VERIFIED` **and** its issuer question is settled — three controls (BBBY, GM, AOL) must adjudicate whether a *second* issuer held their ticker, discharged either by evidencing one or by a cited finding that none was established. **Two of the three are now discharged by evidence.** BBBY carries CIK 886158 (Nasdaq, `$.01` par) and CIK 1130713 (NYSE, `$0.0001` par); AOL carries CIK 883780 and CIK 1468516. Both are `identity_break: true`. AOL is the stricter of the two: both its filings name the New York Stock Exchange and the same symbol, and the registrant names are close, so **the CIK is the only discriminator** — which is exactly why no series may run across the two. **GM alone remains open**, and on *mapping quality* rather than completeness: both its issuers are recorded, but its weakest mapping is still `RESOLVED`. **No CIK has been guessed** — every mapping cites a primary source, and the fixture in `controls.py` still carries none. All three counts come from `tradeit edgar controls`, which prints them separately; do not restate any from memory |
