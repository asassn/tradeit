...
def test_the_shipped_state_measures_17_unresolved_and_19_awaiting_verification() -> None:
    ...
    exactly that for MSFT, CSCO, AMZN, SPY, QQQ and ETYS in turn, which is the
    ...
    assert len(unresolved_controls(evidence)) == 17
    assert len(controls_awaiting_manual_verification(evidence)) == 19
    assert sum(1 for r in resolved if r.counts_in_numerator) == 13
    assert sum(1 for r in resolved if r.is_manually_verified) == 11
