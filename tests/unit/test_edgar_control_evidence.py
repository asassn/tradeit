...
def test_the_shipped_state_measures_16_unresolved_and_18_awaiting_verification() -> None:
    ...
    exactly that for MSFT, CSCO, AMZN, SPY, QQQ, ETYS and WBVN in turn, which is
    ...
    assert len(unresolved_controls(evidence)) == 16
    assert len(controls_awaiting_manual_verification(evidence)) == 18
    assert sum(1 for r in resolved if r.counts_in_numerator) == 14
    assert sum(1 for r in resolved if r.is_manually_verified) == 12
